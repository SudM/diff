...
def attach_policies(objs, id_index, md_by_origin, policysets, policies, rows):
    ...
    for pol_id in policies:
        ...
        rows.append({
            ...
            "type": "Policy",
            "name": md.get("name"),
            "full_path_from_root": path_by_ps[parent],               # names
            "full_path_ids_from_root": idpath_by_ps[parent] + " / " + pol_id,  # IDs fixed
        })
...
def parse_toggle_envs(env_files: dict) -> pd.DataFrame:
    all_rows = []
    def walk(obj, current_parts, env_name):
        name_val = obj.get("name") if isinstance(obj, dict) else None
        adjusted_parts = []
        for part in current_parts:
            if part == "tenants" and name_val:
                adjusted_parts.append(str(name_val))
            else:
                adjusted_parts.append(part)
        if isinstance(obj, dict):
            if "name" in obj or "isEnabled" in obj:
                extra_name = []
                if "name" in obj and not (adjusted_parts and adjusted_parts[-1] == obj["name"]):
                    extra_name = [obj["name"]]
                chain_str = "~".join(adjusted_parts + extra_name)
                all_rows.append({
            ...
            for k,v in obj.items():
                if isinstance(v, list):
                    for elem in v: walk(elem, adjusted_parts+[k], env_name)
                elif isinstance(v, dict):
                    walk(v, adjusted_parts+[k], env_name)
        elif isinstance(obj, list):
            for elem in obj:
                walk(elem, adjusted_parts, env_name)
    for env_name, fp in env_files.items():
        with open(fp,"r",encoding="utf-8") as f:
            data=json.load(f)
        toggles=data.get("toggles",{})
        for section,content in toggles.items():
            if isinstance(content,list):
                for item in content:
                    walk(item,[section],env_name)
            elif isinstance(content,dict):
                walk(content,[section],env_name)
    df_toggle=pd.DataFrame(all_rows)
    df_toggle["state"]=df_toggle["isEnabled"].map(lambda x:"ON" if x else ("OFF" if x==False else ""))
    return df_toggle
...
def run_all(pkg_path: str, env_files: dict, out_dir: str):
    # Step 1: Build datasets (policy + action)
    df_policy = build_policypath_from_package(pkg_path)
    df_action = build_actionlink_from_package(pkg_path)
    df = df_policy.merge(df_action, left_on="originId", right_on="Matched OriginID", how="left")
    ...
    mask_not_arr = ~df["subtenancy_subtype"].astype(str).str.startswith("arrangementtype")
    df.loc[mask_subtype & mask_not_arr, "SubTenancy_Mapped"] = df.loc[mask_subtype & mask_not_arr, "subtenancy_subtype"]
    df["SubTenancy_Mapped"] = df["SubTenancy_Mapped"].apply(lambda s: ("" if isinstance(s,str) and s.strip().lower() in TENANCY_KEYWORDS else s))
    ...
    # Step 2: Parse toggle envs
    df_toggle = parse_toggle_envs(env_files)
    df_toggle_check = df_toggle[df_toggle["chain"].str.contains("checkPermissions", case=False, na=False)].copy()
    df_env_pivot = df_toggle_check.pivot_table(index="chain", columns="environment", values="state", aggfunc="first").reset_index()

    # Step 3: Join
    df_join = df.merge(df_env_pivot, left_on="toggle_path", right_on="chain", how="left")
    df_join.drop(columns=["chain"], inplace=True)

    os.makedirs(out_dir, exist_ok=True)
    final_csv=os.path.join(out_dir,"final_with_envs.csv")
    final_html=os.path.join(out_dir,"final_with_envs.html")
    df_join.to_csv(final_csv,index=False)
    write_html_table(df_join,"Final Combined Output",final_html)

    print(f"✅ Final rows: {len(df_join)}")
...
if __name__=="__main__":
    ...
    run_all(args.package,env_files,args.out_dir)
